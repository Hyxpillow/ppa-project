...
struct
  structure UndirectedGraph = UGraph
  fun louvain (g: UGraph.t) : int Seq.t = 
    let
      val m = UGraph.num_edges g
        ...
        let
          ...
          fun f (i) = 
            let
              ...
              val k_i_in = Seq.nth neighbor_comm_delta_weights comm_new
              val sigma_tot = if comm_new == comm_old 
                then Seq.nth comm_weights comm_new 
                else (Seq.nth comm_weights comm_new) - k_i
              val delta_Q = k_i_in - k_i * sigma_tot / 2 / m
            in
              (comm_new, delta_Q)
            end
        in
